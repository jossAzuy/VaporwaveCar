...
namespace FronkonGames.Retro.Spectrum
{
  ...
  public sealed partial class Spectrum
  {
    ...
    private static readonly Spectrum[] NoEffects = new Spectrum[0];
    ...
    public static Spectrum Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is Spectrum)
              return scriptableRendererData.rendererFeatures[i] as Spectrum;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static Spectrum[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is Spectrum)
                  effects.Add(feature as Spectrum);
          }
