...
public class EnemyCarAI2_Bounded : MonoBehaviour
{
    [Header("Target")]
    ...
    [Tooltip("Collider que define el área permitida (ground)")]
    public Collider movementArea;

    [Tooltip("Fuerza que empuja al enemigo hacia adentro cuando sale del área")]
    public float borderForce = 5f;
    ...
    void KeepInsideArea()
    {
        ...
        Vector3 closestPoint = movementArea.ClosestPoint(transform.position);
        Vector3 offset = transform.position - closestPoint;

        // Si está fuera del collider
        if (offset.magnitude > 0.01f)
        {
            Vector3 pushDir = offset.normalized;
            rb.AddForce(pushDir * borderForce, ForceMode.VelocityChange);
        }
    }
}
